    ...
    Object.entries(body).forEach(([key, value]) => {
        if (Array.isArray(value)) {
            ...
            if (!(blob instanceof Blob)) {
                throw new Error('Expected a Blob in the first position of the array, instead got ' +
                    blob);
            }
            if (typeof filename !== 'string') {
                throw new Error('Expected a string in the second position of the array, instead got ' +
                    filename);
            }
